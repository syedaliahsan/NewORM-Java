...
public class SQLFieldWrapper {
  ...
  private static final Pattern SIMPLE_ALIAS_PATTERN = 
      Pattern.compile("^(.*?)\\s+([a-zA-Z_][a-zA-Z0-9_]*)$", Pattern.CASE_INSENSITIVE);
  ...
  /**
   * Simple wrapper for identifiers.
   */
  private static String wrap(String identifier, String prefix, String suffix) {
      if (identifier == null || identifier.trim().isEmpty()) {
          return "";
      }
      return prefix + identifier.trim() + suffix;
  }
  ...
  public static void main(String[] args) {
      ...
      System.out.println("wrapDBField('user', 'concat(firstName, lastName)', '`', '`', '.'): " + 
          wrapDBField("user", "concat(firstName, lastName)", "`", "`", "."));
  }
...
//        "COUNT", "SUM", "AVG", "MIN", "MAX", "CONCAT", "COALESCE", "NULLIF", 
//        "ABS", "ROUND", "CEIL", "FLOOR", "UPPER", "LOWER", "LENGTH", "SUBSTRING",
//        "TRIM", "DATE", "TIME", "TIMESTAMP", "YEAR", "MONTH", "DAY", "NOW", "CURRENT_DATE"
//    ));
//    
//    private static final Set<String> SQL_KEYWORDS = new HashSet<>(Arrays.asList(
//        "SELECT", "FROM", "WHERE", "GROUP", "ORDER", "BY", "HAVING", "JOIN", 
//        "INNER", "LEFT", "RIGHT", "OUTER", "ON", "AS", "AND", "OR", "NOT", 
//        "IN", "BETWEEN", "LIKE", "IS", "NULL", "ASC", "DESC"
//    ));
//
//    /**
//     * Wraps an array of SQL field expressions with the specified prefix and suffix.
...
//        }
//        
//        // Split by commas but preserve quoted strings and parentheses
...
//                parenDepth--;
//                currentField.append(c);
//            } else if (c == ',' && !inSingleQuotes && !inDoubleQuotes && parenDepth == 0) {
//                fieldList.add(currentField.toString().trim());
//                currentField = new StringBuilder();
//            } else {
//                currentField.append(c);
//            }
...
//        }
//        
//        String[] wrappedArray = wrapFields(fieldList.toArray(new String[0]), prefix, suffix);
//        return String.join(", ", wrappedArray);
//    }
//    
//    /**
...
//        
//        // First, check if the entire expression is just a simple identifier
//        if (isSimpleIdentifier(expr) && !isSQLFunction(expr)) {
//            return wrapIdentifier(expr, prefix, suffix);
//        }
//        
//        // Check for ORDER BY clause first
...
//            if (Character.isDigit(c) || (c == '-' && i + 1 < length && Character.isDigit(expr.charAt(i + 1)))) {
//                StringBuilder number = new StringBuilder();
//                while (i < length && (Character.isDigit(expr.charAt(i)) || expr.charAt(i) == '.' || expr.charAt(i) == '-')) {
...
//                if (tempPos < length && expr.charAt(tempPos) == '(') {
//                    // This is a function name - don't wrap it
//                    result.append(possibleFunction.toString());
//                    
//                    // Skip whitespace
//                    while (i < length && Character.isWhitespace(expr.charAt(i))) {
//                        result.append(expr.charAt(i));
...
//                    // Not a function, could be an identifier or operator
//                    i = startPos; // Reset to start position
//                    result.append(parseToken(expr, i, prefix, suffix));
//                    // Update i to after the token
//                    while (i < length && (Character.isLetterOrDigit(expr.charAt(i)) || expr.charAt(i) == '_' || expr.charAt(i) == '.')) {
//                        i++;
//                    }
//                }
//                continue;
...
//        
//        // Simple identifier
//        if (isSimpleIdentifier(tokenStr) && !isSQLFunction(tokenStr) && !isSQLKeyword(tokenStr)) {
//            return prefix + tokenStr + suffix;
//        }
//        
//        return tokenStr;
//    }
